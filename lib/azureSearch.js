...
const AZURE_CONFIG = {
  ...
  embeddingDimensions: 3072, // text-embedding-3-large has 3072 dimensions
...
export async function generateEmbeddings(text) {
  ...
  if (!endpoint || !apiKey) {
    throw new Error('Missing Azure OpenAI configuration. Please set AZURE_OPENAI_ENDPOINT and AZURE_OPENAI_API_KEY environment variables.');
  }
  ...
  const url = `${endpoint}/openai/deployments/${AZURE_CONFIG.embeddingModel}/embeddings?api-version=2024-02-15-preview`;
  ...
  try {
    ...
    if (includeTextSearch) {
      ...
      searchOptions.semanticSearchOptions = {
        configurationName: 'hoa-semantic-config',
      };
    }
